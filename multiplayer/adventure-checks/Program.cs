...
Check(aresdenConfig.DwellAreas!.Length>=17&&elvineConfig.DwellAreas!.Length>=17&&aresdenConfig.DwellAreas.Sum(p=>p.Count)>=210&&elvineConfig.DwellAreas.Sum(p=>p.Count)>=210&&aresdenConfig.DwellAreas.Concat(elvineConfig.DwellAreas).All(p=>p.Area!.X2<=69||p.Area.X1>=250||p.Area.Y2<=48||p.Area.Y1>=240),"Aresden and Elvine have dense beginner hunting pits exclusively around their outer perimeters");
