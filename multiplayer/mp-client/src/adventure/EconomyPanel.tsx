...
export function EconomyPanel(){
 ...
 const [search,setSearch]=useState(''),[category,setCategory]=useState('all'),[affordable,setAffordable]=useState(false);
 useEffect(()=>{setSearch('');setCategory('all');setAffordable(false);},[stats?.service]);
 ...
 const rows=offers.filter(o=>o.service===stats.service&&o.name.toLocaleLowerCase().includes(search.toLocaleLowerCase())&&(category==='all'||(category==='weapon'?getItemById(o.itemId)?.itemType==='weapon':category==='armor'?['armor','shield','hauberk','leggings','helmet','boots','cape'].includes(getItemById(o.itemId)?.itemType??''):o.spellId>=0))&&(!affordable||(stats.gold>=o.price&&stats.level>=o.level&&stats.intelligence>=o.intelligence))).sort((a,b)=>a.level-b.level||a.price-b.price);
 ...
 return createPortal(<div className="economy-overlay" onMouseDown={e=>{if(e.target===e.currentTarget)closeEconomyPanel()}}><section className={`economy parchment-${stats.service}`} role="dialog" aria-modal="false" aria-label={title}>
  ...
  <div className="economy-body">
   {stats.service!=='warehouse'&&<div className="economy-toolbar"><input aria-label="Buscar en la tienda" placeholder="Buscar por nombre…" value={search} onChange={e=>setSearch(e.target.value)}/><select aria-label="Categoría" value={category} onChange={e=>setCategory(e.target.value)}><option value="all">Todo el catálogo</option><option value="weapon">Armas</option><option value="armor">Armaduras</option><option value="spell">Hechizos</option></select><label><input type="checkbox" checked={affordable} onChange={e=>setAffordable(e.target.checked)}/> A mi alcance</label><span className="economy-result-count">{rows.length} opciones</span></div>}
