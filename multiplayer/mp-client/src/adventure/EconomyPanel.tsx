...
export function EconomyPanel(){
 ...
 const [offers,setOffers]=useState<Offer[]>([]),[open,setOpen]=useState(true);
 useEffect(()=>{const c=new AbortController();fetch('/api/game/economy',{signal:c.signal}).then(r=>r.ok?r.json():Promise.reject()).then(r=>setOffers(r.offers??[])).catch(()=>{});return()=>c.abort();},[]);
 ...
 return <section className="economy" aria-label={title}>
  ...
  {open&&<div className="economy-body">
   ...
   {stats.service==='warehouse'&&<><h3>Mochila → almacén</h3>{bag.map(i=><div className="economy-row" key={i.itemUid}><span>{byItem.get(i.itemId)?.name??`Objeto ${i.itemId}`}<small>Cantidad {i.quantity??1}</small></span><button onClick={()=>send('deposit','',i.itemUid)}>Guardar</button></div>)}<h3>Almacén → mochila</h3>{stats.warehouse.map(i=><div className="economy-row" key={i.itemUid.toString()}><span>{byItem.get(i.itemId)?.name??`Objeto ${i.itemId}`}<small>Cantidad {i.quantity??1}</small></span><button onClick={()=>send('withdraw','',i.itemUid.toString())}>Retirar</button></div>)}</>}
