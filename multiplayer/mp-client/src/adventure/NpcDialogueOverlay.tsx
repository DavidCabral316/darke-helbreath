...
export function NpcDialogueOverlay(){
    ...
    useEffect(()=>{
        if(service&&service!==lastService.current){lastService.current=service;setVisible(true)}
        if(!service){lastService.current='';setVisible(false)}
    },[service]);
    ...
    return createPortal(<div className={`npc-cinematic ${character.accent}`} role="dialog" aria-modal="false" aria-label={`Diálogo con ${character.name}`}>
        ...
        <section className="npc-cinematic-box">
            ...
            <p>{character.line}</p>
            <button onClick={()=>setVisible(false)}>Ver sus servicios <span>◆</span></button>
        </section>
    </div>,target);
}
