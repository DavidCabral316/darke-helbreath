...
type Bonus={damage:number;defense:number;level:number;magic:number;mana:number;attackSpeed:number;itemLevel:number};
export function InventoryItemHoverOverlay(){
    ...
    const itemLevel=value(ItemEffect.AFFIX_ITEM_LEVEL)||(bonus?.itemLevel??1);
    ...
    const previousEffects=equipped?.effectOverrides??[];
    const previousItemLevel=previousEffects.find(e=>e.effect===ItemEffect.AFFIX_ITEM_LEVEL)?.effectColor||(previous?.itemLevel??0);
    const itemLevelDelta=itemLevel-previousItemLevel;
    ...
    return createPortal(<div role="tooltip" style={{position:'fixed',left:Math.max(8,Math.min(info.mouseX+15,window.innerWidth-278)),top:Math.max(8,Math.min(info.mouseY+20,window.innerHeight-260)),zIndex:30000,pointerEvents:'none',background:'#21170ffa',border:'1px solid #927341',borderRadius:6,boxShadow:'0 4px 16px #0009'}}>
        ...
        {!!bonus&&<p style={{fontWeight:800,color:'#f4cc69'}}>iLvl {itemLevel}{!alreadyEquipped&&previousItemLevel>0&&<span className={itemLevelDelta>0?'item-gain':itemLevelDelta<0?'item-loss':''}> · {itemLevelDelta>0?'+':''}{itemLevelDelta} vs. equipado</span>}</p>}
