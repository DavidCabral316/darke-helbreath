...
type Sheet = { frames: Frame[]; png: Blob };
const cache = new Map<string, Promise<Sheet[]>>();
function load(name: string): Promise<Sheet[]> {
    ...
    const promise = fetch(`/assets/sprites/${name}.spr`).then(async response => {
        ...
        const metadata: { frames: Frame[]; length: number }[] = [];
    ...
    useEffect(() => {
        ...
        async function render() {
            ...
            const sheets = await Promise.all(layers.map(async layer => {
                const sheet = (await load(layer.name))[layer.sheet];
