...
function Home({ account, status }: { account: Account | null | undefined; status: { online: boolean; players: number } | null }) {
    return <><section className="hero">
        <div className="hero-art" aria-hidden="true" /><div className="hero-grid" aria-hidden="true" />
        <div className="hero-copy"><p className="eyebrow"><span /> EL LEGADO VUELVE A VIVIR</p><h1>Volvé a un mundo<br />que nunca <em>olvidaste.</em></h1>
            <p className="hero-description">Elegí tu reino, dominá la magia y escribí una nueva leyenda en un mundo persistente inspirado en el Helbreath clásico.</p>
        ...
        </div><div className="hero-sigil" aria-hidden="true"><div><span>D</span></div><small>ALFA · TEMPORADA CERO</small></div><span className="hero-caption">ARESDEN / ELVINE &nbsp; · &nbsp; UN MUNDO POR REDESCUBRIR</span>
    </section>
    <section className="world-pulse" aria-label="Características del mundo"><div><strong>200</strong><span>Niveles de progresión</span></div><div><strong>16+</strong><span>Regiones conectadas</span></div><div><strong>∞</strong><span>Historias posibles</span></div><div><strong>24/7</strong><span>Mundo persistente</span></div></section>
    <section className="feature-strip"><article><b>01</b><span>SIN DESCARGAS</span><h2>Tu navegador es el portal.</h2><p>Creá tu cuenta, elegí tu personaje y entrá directamente al mundo.</p></article><article><b>02</b><span>DOS DESTINOS</span><h2>Jurá lealtad a tu ciudad.</h2><p>Aresden o Elvine: dos identidades, una rivalidad y un territorio por conquistar.</p></article><article><b>03</b><span>BOTÍN ÚNICO</span><h2>Cada hallazgo puede cambiarte.</h2><p>Conseguí equipo especial, afijos poco comunes y piezas que merecen convertirse en leyenda.</p></article></section>
    <section className="news"><div className="news-visual"><span>EL MUNDO DE DARKE</span></div><article><p className="eyebrow"><span /> TEMPORADA CERO</p><h2>Dos reinos.<br /><em>Una tierra herida.</em></h2><p>Aresden y Elvine disputan el corazón de Middleland mientras criaturas antiguas despiertan en sus fronteras. Empezá en las afueras de tu ciudad, reuní oro y preparate para cruzar sus portales.</p><ul><li>Combate y progresión persistente</li><li>Economía, equipo y magia</li><li>Mapas clásicos con una nueva identidad</li></ul><Link className="secondary-link" to="/guide">Conocer los primeros pasos →</Link></article></section>
    <section className="worlds-showcase"><header><p className="eyebrow"><span /> ELEGÍ TU ORIGEN</p><h2>Tu bandera define el comienzo.<br /><em>Tus acciones, el destino.</em></h2></header><div><article className="aresden-card"><span>REINO DEL ACERO</span><h3>Aresden</h3><p>Fortaleza, disciplina y una voluntad forjada en guerra.</p></article><article className="elvine-card"><span>REINO ARCANO</span><h3>Elvine</h3><p>Conocimiento, magia y secretos que atraviesan los siglos.</p></article></div></section></>;
}
...
function Characters() {
    ...
    return <section className="characters-page"><div className="characters-backdrop" aria-hidden="true"/><div className="section-heading"><div><p className="eyebrow"><span /> SALÓN DE LOS HÉROES</p><h1>Elegí tu leyenda.</h1><p>Tres destinos posibles. El mundo recordará cuál continuaste hoy.</p></div><Link className="account-pill" to="/account">Mi cuenta <span>→</span></Link></div>
        {error && <Message>{error}</Message>}{!ready ? <Loading /> : <div className="character-grid">{characters.map(c => <article className={`character-card ${c.deletedAt ? 'deleted' : ''}`} key={c.id}>
            <div className="character-card-shine"/><div className="character-meta"><span className="tag">{c.town.toUpperCase()}</span><strong>NIVEL {c.level}</strong></div><CharacterPreview gender={c.gender} skin={c.skin} hair={c.hair} clothes={c.clothes} equippedItems={previewEquipment(c)} /><div className="character-copy"><small>{c.town === 'aresden' ? 'JURAMENTO DEL ACERO' : 'PACTO ARCANO'}</small><h2>{c.name}</h2><p>{c.deletedAt ? 'Eliminación pendiente · recuperable por 3 días' : `Última ubicación · ${c.world}`}</p></div>
        ...
        </article>)}{Array.from({ length: Math.max(0, 3 - characters.length) }, (_, i) => <Link className="empty-character" key={i} to="/characters/new"><span>＋</span><small>RANURA DISPONIBLE</small><h2>Una historia<br/>por escribir.</h2><p>Crear nuevo personaje →</p></Link>)}</div>}
